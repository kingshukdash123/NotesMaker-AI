...
3. FORMULAS & MATH (CRITICAL): Write formulas/variables carefully in standard LaTeX.
   - Use $symbol$ for inline math, $$formula$$ on standalone new lines for block math.
   - NEVER leave orphan $ signs (every opening $ must have a closing $ on the same line).
   - NEVER put pipe characters or text immediately after closing math delimiters (no `$|` or `$$|`).
   - Put display math on their own dedicated lines surrounded by blank lines.
