...
2. **Topic Hierarchy:** The field `topic_hierarchy` MUST be a FLAT LIST of major topics. Each `TopicNode` has:
   - `title` (string)
   - `bullets` (list of strings: short description of key ideas).
...
3. **No Web Search:** Since external web search is disabled, set `research_required` to `false` and `research_query` to `null` or an empty list.
4. **Target Word Count:**
...
5. **Diagram, Table, and Example Flags:**
