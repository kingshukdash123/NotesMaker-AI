...
export default function RightAssistantSidebar({ currentUser, isOpen, onClose, mode = 'sidebar', onToggleMode, onZoom }) {
  ...
  const containerClasses = effectiveMode === 'sidebar'
    ? `fixed top-[53px] bottom-0 right-0 w-[calc(100vw-1rem)] max-w-sm sm:w-96 lg:static lg:h-full z-[85] flex flex-col shrink-0 overflow-hidden transition-all duration-300 ease-in-out ${isOpen
  ...
  return (
    <>
      ...
      {effectiveMode === 'sidebar' && (
        <div
          onClick={onClose}
          className={`fixed inset-0 bg-black/60 backdrop-blur-[1px] z-[84] lg:hidden mt-[53px] transition-opacity duration-300 ease-in-out ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
            }`}
        />
      )}

      <div key={effectiveMode} className={containerClasses}>
        <div className={`${effectiveMode === 'sidebar'
            ? 'w-[calc(100vw-1rem)] max-w-sm sm:w-96 lg:w-80 xl:w-96'
            : 'w-full'
          } h-full flex flex-col min-w-0 relative`}>
          {/* Header */}
          <div className={`px-3 sm:px-4 py-2 sm:py-3 border-b flex items-center justify-between shrink-0 h-11 sm:h-[50px] md:h-[53px] ${isDark ? 'border-zinc-900 bg-zinc-950' : 'border-zinc-200 bg-white'
            }`}>
            {isLoadingHistory ? (
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 min-w-0 flex-1">
                ...
                <button
                  ...
                  className={`flex items-center gap-1 min-w-0 text-left cursor-pointer transition ${isDark ? 'hover:text-zinc-200' : 'hover:text-zinc-900'
                    }`}
                >
          ...
          <div className="flex-1 relative min-h-0 flex flex-col">
            ...
            {showSelector && (
              <div className={`absolute inset-0 z-[130] flex flex-col animate-in fade-in duration-150 ${isDark ? 'bg-zinc-950' : 'bg-white'
                }`}>
                    ...
                    threads.map((t) => {
                      ...
                      return (
                        <div
                          ...
                          className={`flex items-center justify-between p-2.5 rounded-xl transition duration-150 cursor-pointer ${isActive
                              ? isDark
                        ...
                        >
                          <div className="flex items-center gap-2 min-w-0 flex-1 mr-2">
                            <FileText className={`w-3.5 h-3.5 shrink-0 select-none ${isActive
            ...
            <div className={`flex-1 overflow-y-auto px-2.5 sm:px-4 py-2 sm:py-3 space-y-2 sm:space-y-3 custom-scrollbar ${isDark ? 'bg-black/5' : 'bg-white'
              }`}>
              {isLoadingHistory ? (
                    ...
                    <div className={`space-y-1 sm:space-y-1.5 border-t pt-3 sm:pt-4 flex flex-col ${isDark ? 'border-zinc-900' : 'border-zinc-200'}`}>
                      {COMPACT_SUGGESTIONS.map((sug) => (
                        <button
                          ...
                          className={`text-left px-2 py-1.5 sm:px-2.5 sm:py-2 rounded-lg border text-[9px] sm:text-[10px] transition cursor-pointer ${isDark
            ...
            {activeThread && !showSelector && (
              <div className="p-2 sm:p-2.5 shrink-0 bg-transparent">
                <div className={`rounded-xl px-2 py-1 flex items-center border transition-colors ${isDark ? 'bg-zinc-900/60 border-zinc-800 focus-within:border-zinc-700' : 'bg-zinc-100 border-zinc-200 focus-within:border-zinc-300 shadow-xs'
                  }`}>
                  <ChatInput
                    value={inputValue}
      ...
      >
        <div className="space-y-1.5">
          ...
          <input
            ...
            className={`w-full rounded-xl px-3 py-2 text-xs transition focus:outline-none focus:border-orange-500 ${isDark
