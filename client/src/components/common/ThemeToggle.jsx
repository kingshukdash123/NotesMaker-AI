...
export default function ThemeToggle({ 
  ...
  if (variant === 'button') {
    return (
      <button
        ...
        className={`w-full flex items-center justify-between px-4 py-2.5 rounded-xl transition-all duration-200 cursor-pointer ${
          isDark
            ? 'bg-zinc-900/60 border border-zinc-800 text-zinc-300 hover:border-zinc-700 hover:text-zinc-100 hover:bg-zinc-800/60'
            : 'bg-zinc-50 text-zinc-800 hover:bg-zinc-100 shadow-xs'
        } ${className}`}
      >
        <div className="flex items-center gap-2.5">
          <div className={`p-1.5 rounded-lg ${isDark ? 'bg-zinc-800 text-orange-400' : 'bg-zinc-900 text-white'}`}>
          ...
          <div className="text-left">
            ...
            <p className={`text-[10px] mt-1 ${isDark ? 'text-zinc-500' : 'text-zinc-500'}`}>
              {isDark ? 'Deep dark workspace' : 'Clean light workspace'}
            </p>
          </div>
        </div>
  ...
  // Default: Compact Icon button for Header / Navigation (matches Header Profile & Zoom buttons)
  return (
    <button
      type="button"
      onClick={toggleTheme}
