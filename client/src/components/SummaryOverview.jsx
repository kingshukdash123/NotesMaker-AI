  ...
  if (!result || !result.lecture_outline) {
    return (
      <div className={`max-w-3xl mx-auto text-center py-16 px-6 rounded-2xl border my-8 space-y-4 shadow-sm ${
        isDark ? 'bg-black border-zinc-800' : 'bg-white border-zinc-200 shadow-xs'
      }`}>
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center mx-auto border ${
          isDark ? 'bg-zinc-950 border-zinc-900 text-orange-400' : 'bg-orange-500/10 border-orange-500/20 text-orange-600'
        }`}>
          <LayoutDashboard className="w-6 h-6" />
        </div>
        <h3 className={`text-base font-bold ${isDark ? 'text-zinc-200' : 'text-zinc-900'}`}>
          No lecture summary available
        </h3>
        <p className={`text-xs max-w-md mx-auto leading-relaxed ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-6 xl:grid-cols-3">
        ...
        <div className="space-y-4 sm:space-y-6 xl:col-span-2">
          ...
          <div className={`rounded-2xl p-4 sm:p-6 shadow-sm space-y-3 sm:space-y-4 border ${
            isDark ? 'border-zinc-800 bg-black' : 'border-zinc-200 bg-white shadow-xs'
          }`}>
            <div className={`flex items-center gap-2 pb-3 border-b ${
              isDark ? 'border-zinc-900 text-orange-400' : 'border-zinc-100 text-zinc-900 font-bold'
            }`}>
                  ...
                  <div key={index} className="flex items-start gap-2.5">
                    <input 
                      ...
                      className={`mt-0.5 accent-orange-500 cursor-default shrink-0 w-3.5 h-3.5 rounded ${
                        isDark ? 'border-zinc-900 bg-zinc-950' : 'border-zinc-300 bg-white'
                      }`}
                    />
